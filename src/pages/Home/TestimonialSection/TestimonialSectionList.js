...
function TestimonialList() {
  ...
  const getTestimonial = async () => {
    try {
      const response = await api.get("/api/user/testimonial");
  ...
  const handleDelete = async (id) => {
    try {
      ...
      if (confirmDelete) {
        await api.delete(`/api/user/testimonial/${id}`);
        getTestimonial();
      }
    } catch (error) {
  ...
  const filteredDataList = testimonialList.filter(
    (data) =>
      ...
      ((data.position && data.position.toLowerCase()) || "").includes(
        searchQuery.toLowerCase()
      )
  );
  ...
  const toggleDisplayOnPage = async (_id) => {
    try {
      const updatedDataList = testimonialList.map((data) => {
        if (data._id === _id) {
          return { ...data, displayOnPage: !data.displayOnPage };
        }
        return data;
      });
      setTestimonialList(updatedDataList);

      const DataToUpdate = testimonialList.find(
        (data) => data._id === _id
      );
      const updateddata = {
        ...DataToUpdate,
      ...
      await api.put(`/api/user/testimonial/${_id}`, updateddata);
    } catch (error) {
      console.log(error);
    }
  };
  ...
  return (
    <div>
      ...
      <div className="d-sm-flex align-items-center justify-content-between mb-4">
        <h1 className="h3 mb-0 text-gray-800">Testimonial Section List :</h1>
        <Link
          to="/portal/testimonial-create"
                ...
                <tbody>
                  {filteredDataList.map((data, index) => (
                    <tr key={data._id}>
                      ...
                      <td>
                        <Link
                          to={`/portal/testimonial-edit/${data._id}`}
