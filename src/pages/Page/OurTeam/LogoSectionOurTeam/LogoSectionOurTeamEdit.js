...
function LogoSectionOurTeamEdit() {
  ...
  const getData = async () => {
    try {
      const response = await api.get(
        `/api/user/logoSectionDataOurTeam/${params.id}`
      );
  ...
  const myFormik = useFormik({
    ...
    onSubmit: async (values) => {
      try {
        setLoading(true);
        await api.put(`/api/user/logoSectionDataOurTeam/${params.id}`, values);
        setLoading(false);
        navigate("/portal/logoSectionDataOurTeam-list");
      } catch (error) {
        console.log(error);
        setLoading(false);
      }
  ...
  const uploadImage = async (file) => {
    ...
    formData.append("folderName", "images/OurTeam_logoSection_pic")
...
export default LogoSectionOurTeamEdit;
