...
function TeamMemberIntroList() {
  ...
  const getData = async () => {
    try {
      const response = await api.get("/api/user/teamintro");
  ...
  const handleDelete = async (id) => {
    try {
      ...
      if (confirmDelete) {
        await api.delete(`/api/user/teamintro/${id}`);
        getData();
      }
    } catch (error) {
  ...
  const filteredDataList = dataList.filter(
    (data) =>
      ...
      ((data.role && data.role.toLowerCase()) || "").includes(
        searchQuery.toLowerCase()
      )
  );
  ...
  const toggleDisplayOnPage = async (_id) => {
    try {
      ...
      await api.put(`/api/user/teamintro/${_id}`, updatedData);
    } catch (error) {
      console.log(error);
    }
  };
  ...
  return (
    <div>
      ...
      <div className="d-sm-flex align-items-center justify-content-between mb-4">
        <h1 className="h3 mb-0 text-gray-800"> Team Intro Data List :</h1>
        <Link
          to="/portal/team-intro-create"
...
export default TeamMemberIntroList;
