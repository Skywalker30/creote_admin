...
function HeaderList() {
  ...
  const handleDelete = async (id) => {
    try {
      ...
      if (confirmDelete) {
        await api.delete(`/api/user/header/${id}`);
        getData();
      }
    } catch (error) {
  ...
  const filteredDataList = dataList.filter(
    (data) =>
      ((data.location && data.location.toLowerCase()) || "").includes(
        searchQuery.toLowerCase()
      ) ||
      ((data.email && data.email.toLowerCase()) || "").includes(
        searchQuery.toLowerCase()
      ) ||
      ((data.phone) || "").includes(searchQuery.toLowerCase())
  );
  ...
  const toggleDisplayOnPage = async (_id) => {
    try {
      ...
      await api.put(`/api/user/header/${_id}`, updatedData);
    } catch (error) {
      console.log(error);
    }
  };
  ...
  return (
    <div>
      ...
      <div className="d-sm-flex align-items-center justify-content-between mb-4">
        <h1 className="h3 mb-0 text-gray-800">Header List :</h1>
        <Link
          to="/portal/header-create"
                ...
                <tbody>
                  {filteredDataList.map((data, index) => (
                    <tr key={data._id}>
                      ...
                      <td>
                        <Link
                          to={`/portal/header-edit/${data._id}`}
