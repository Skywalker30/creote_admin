...
function TeamMembersEdit() {
  ...
  const getUserData = async () => {
    try {
      const response = await api.get(`/api/user/teamMembers/${params.id}`);
  ...
  const myFormik = useFormik({
    ...
    onSubmit: async (values) => {
      try {
        setLoading(true);
        await api.put(`/api/user/teamMembers/${params.id}`, values);
        setLoading(false);
  ...
  const uploadImage = async (file) => {
    ...
    try {
      ...
      setInitialUserData((prevUserData) => ({
        ...prevUserData,
